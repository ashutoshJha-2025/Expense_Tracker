...
const Login = () => {
    ...
    const sendData = async () => {
        try {
            const result = await axios.post('https://expense-tracker-backend-jigy.onrender.com/api/user/authentication/login', formData, { withCredentials: true })
        ...
        <div className="w-full h-full flex items-center justify-center sm:px-4">
            <div className="w-100 max-sm:w-75 bg-white rounded-3xl shadow-2xl sm:px-8 max-sm:px-4 py-4 ">
                ...
                <form onSubmit={(e) => e.preventDefault()} className="mt-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3 ">
                        <p>Don't have an account? <button onClick={() => navigate('/register')} className='text-sky-600 font-semibold cursor-pointer '>Sign up</button></p>
                    </div>
                </form>
            </div>
        </div>
